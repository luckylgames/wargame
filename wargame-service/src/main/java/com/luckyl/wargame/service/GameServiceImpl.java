...
import com.luckyl.wargame.AttackState;
...
public class GameServiceImpl implements GameService {
    ...
    //checked
    private AttackState attackOutcome(Piece myPiece, Piece opponent) {
        int state = myPiece.compareTo(opponent);

        if(state > 0) return AttackState.WIN;
        else if(state < 0 ) return AttackState.LOSE;
        return AttackState.TIE;     //default
    }
}
